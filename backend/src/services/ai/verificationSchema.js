...
/*
 * This schema is the boundary for future provider output. It mirrors the
 * persisted VerificationResult contract so arbitrary provider JSON is not
 * written directly to MongoDB.
 */
...
export function validateAiVerificationResult(output) {
  return aiVerificationResultSchema.validate(output, {
    abortEarly: false,
    convert: true,
    stripUnknown: true,
  });
...
export function isEvidenceTraceable(
  evidence,
  suppliedEvidence = []
) {
  const availableEvidence = Array.isArray(suppliedEvidence)
    ? suppliedEvidence
    : [];
  ...
  return evidence.every((item) => {
    ...
    return availableEvidence.some((supplied) => {
      const populatedFields = identityFields.filter(
        (field) => item[field] !== null && item[field] !== undefined
      );

      return supplied &&
        supplied.evidenceId === item.evidenceId &&
        populatedFields.every(
        (field) =>
          supplied[field] === item[field]
        );
    });
  });
}
...
function attachSourceIds(evidence, suppliedEvidence) {
  return evidence.map((item) => {
    const supplied = suppliedEvidence.find(
      (candidate) =>
...
export function normalizeAiVerificationResult(
  output,
  { suppliedEvidence = [] } = {}
) {
  ...
  if (
    ...
    return {
      error: new Error(
        "AI evidence could not be traced to supplied evidence."
      ),
      value: null,
    };
  }

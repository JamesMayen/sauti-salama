            ...
            <>
              ...
              {selected.result ? (
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  ...
                  <div><p className="text-xs uppercase tracking-wide text-slate-400">Evidence sufficiency</p><p className="mt-1 font-semibold text-white">{label(selected.result.evidenceSufficiency)}</p></div>
                  <div><p className="text-xs uppercase tracking-wide text-slate-400">Claim type</p><p className="mt-1 font-semibold text-white">{selected.result.claimType ? label(selected.result.claimType) : "Not classified"}</p></div>
                  ...
                  {selected.result.retrievalMethod && <div><p className="text-xs uppercase tracking-wide text-slate-400">Retrieval method</p><p className="mt-1 font-semibold text-white">{selected.result.retrievalMethod === "online_search" ? "Online search" : selected.result.retrievalMethod === "mixed" ? "Online + registry" : "Source registry"}</p></div>}
                  {selected.result.technicalFailure && <div><p className="text-xs uppercase tracking-wide text-slate-400">Technical failure</p><p className="mt-1 font-semibold text-red-300">Yes — verification was temporarily unavailable</p></div>}
                  ...
                  {selected.result.evidence?.length > 0 && <div className="sm:col-span-2"><p className="text-xs uppercase tracking-wide text-slate-400">Evidence</p><div className="mt-2 grid gap-3 sm:grid-cols-2">{selected.result.evidence.map((item, index) => <div key={item.evidenceId || index} className="rounded-lg border border-slate-700 bg-slate-950 p-3"><div className="flex items-start justify-between gap-2"><p className="text-sm font-medium text-white">{item.title || item.source || "Source"}</p><span className="rounded-full bg-slate-800 px-2 py-0.5 text-[10px] font-medium text-slate-400">Tier {item.sourceTier || "?"}</span></div><p className="mt-1 text-xs text-slate-400">{item.sourceType || "Source type unavailable"}{item.date ? ` | ${item.date}` : ""}{item.publisher ? ` | Publisher: ${item.publisher}` : ""}</p>{item.relationshipToClaim && <p className="mt-1 text-xs text-slate-400">Relationship: {item.relationshipToClaim.replace(/_/g, " ")}</p>}{item.relevance && <p className="mt-2 text-sm text-slate-300">{item.relevance}</p>}{item.url && /^https?:\/\//i.test(item.url) && <a href={item.url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-block break-all text-xs font-semibold text-emerald-300 underline">Open source</a>}</div>)}</div></div>}
                </div>
